...
def save_config(cats):
    ...
    CONFIG_PATH.write_text(json.dumps(cats, indent=4))
...
def make_extension_widget(exts, selected, on_change):
    ...
    if isinstance(exts, dict):
        # grid layout for "All" grouped categories
        ...
        for e in exts:
            ...
            cb.clicked.connect(lambda _, x=e: on_change(x))
            row.addWidget(cb)
        row.addStretch(); content.setLayout(row)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        btn_b = QPushButton("Browse..."); btn_b.setIcon(QIcon.fromTheme("folder-open"))
        ...
        orow.addWidget(self.rec); orow.addWidget(self.del_empty)
        ...
        # organize button
        btn_o = QPushButton("Organize Now"); btn_o.setIcon(QIcon.fromTheme("system-run"))
        btn_o.setFixedHeight(40); btn_o.clicked.connect(self.run_organizer)
        main.addWidget(btn_o)

        # presets toolbar
        prow = QHBoxLayout()
        prow.addWidget(QLabel("Preset:"))
        self.pcombo = QComboBox(); self.pcombo.addItems(self.presets.keys())
        ...
        b2 = QHBoxLayout()
        ba = QPushButton("Add Category"); ba.clicked.connect(self.add_category)
        br = QPushButton("Remove Category"); br.clicked.connect(self.remove_category)
        b2.addWidget(ba); b2.addWidget(br)
        b2.addItem(QSpacerItem(20,20,QSizePolicy.Expanding,QSizePolicy.Minimum))
        cgl.addLayout(b2)
        main.addWidget(cg)

        # status message label
        self.status = QLabel(""); self.status.setStyleSheet("color:#4caf50;font-weight:bold;")
    ...
    def run_organizer(self):
        ...
        d = self.path.text()
        if not d: return self.show_status("Choose folder first.", True)
    ...
    def add_preset(self):
        # create a new preset from current categories
        ...
        if ok and name.strip():
            self.presets[name.strip()] = self.cats.copy()
            save_presets({k:v for k,v in self.presets.items() if k!="Default"})
            self.pcombo.addItem(name.strip()); self.pcombo.setCurrentText(name.strip())
            self.show_status(f"Added preset '{name.strip()}'")

    def rename_preset(self):
        # rename currently selected preset
        cur = self.pcombo.currentText()
        if cur=="Default": return self.show_status("Cannot rename Default.", True)
        ...
        if ok and new.strip() and new.strip()!=cur:
            ...
            save_presets({k:v for k,v in self.presets.items() if k!="Default"})
            idx = self.pcombo.currentIndex(); self.pcombo.setItemText(idx,new.strip()); self.show_status(f"Renamed preset to '{new.strip()}'")

    def save_preset(self):
        # overwrite existing preset
        cur = self.pcombo.currentText()
        if cur=="Default": return self.show_status("Use 'Add Preset' to create.", True)
        self.presets[cur] = self.cats.copy()
        save_presets({k:v for k,v in self.presets.items() if k!="Default"})
        self.show_status(f"Saved preset '{cur}'")

    def delete_preset(self):
        # remove selected preset file
        cur = self.pcombo.currentText()
        if cur=="Default": return
        self.presets.pop(cur, None)
        save_presets({k:v for k,v in self.presets.items() if k!="Default"})
        self.pcombo.clear(); self.pcombo.addItems(self.presets.keys()); self.load_preset("Default"); self.show_status(f"Deleted preset '{cur}'")
...
def organize_folder(folder: Path, cats: dict, recursive: bool):
    ...
    for f in files:
        ...
        for cat, exts in cats.items():
            if ext in exts:
                dest = folder/cat; dest.mkdir(exist_ok=True)
                shutil.move(str(f), str(dest/f.name)); break
        else:
            oth = folder/"Others"; oth.mkdir(exist_ok=True)
